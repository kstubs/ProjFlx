<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:wbt="myWebTemplater.1.0" exclude-result-prefixes="wbt">
	...
	<xsl:template match="wbt:query" mode="identity-translate">
		...
		<xsl:variable name="force-grid-view" select="$results/schema/query/@force-grid-view='true'"/>
		...
		<xsl:if test="$numrows = 0 or $numrows > 1">
			<div>
			...
			<div class="row clear" id="search_options">
				...
				<div class="col-md-4">
					<xsl:choose>
						<xsl:when test="@action='NonQuery' or @action='Static'">
		...
		<div class="clear tab-control">
			<ul class="nav nav-tabs" role="tablist">
				<li role="presentation" class="active">
					<a href="#home" aria-controls="home" role="tab" data-toggle="tab"><xsl:value-of select="$query"/></a>
				</li>				
				<xsl:choose>
	...
	<xsl:template match="row" mode="wbt:default-table-multirow.link">
		<th><a href="#"><i class="fa fa-link"/></a></th>		
	</xsl:template>
